...
class Workflow2Graph {
  ...
  _convert(wfe = {}, meta = {}) {
    ...
    tasks.forEach(tt => {
      if (tt.taskType === 'FORK') {
        const wfts = [];
        const forkedTasks = (tt.inputData && tt.inputData.forkedTasks) || [];
    ...
    for (const v in this.vertices) {
      const et = this.executedTasks[v];
      const status = et ? et.status : '';
  ...
  getNodes(vertices, nodes, t1, t2, forks, subworkflows, isExecutingCase) {
    const executed = 'stroke: #000000; fill: transparent';
    const defstyle = 'stroke: #ccc; fill: transparent; stroke-dasharray: 5, 5';
    ...
    if (t1.type === 'FORK_JOIN') {
      ...
      for (const k in t1.decisionCases) {
