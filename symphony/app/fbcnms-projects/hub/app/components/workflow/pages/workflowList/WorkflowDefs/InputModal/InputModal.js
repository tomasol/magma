...
const getInputs = def => {
  const matchArray = def.match(/(?<=workflow\.input\.)([a-zA-Z0-9-_]+)/gim);
  return [...new Set(matchArray)];
};

const getDetails = (def, inputsArray) => {
  const [detailsArray, tmpDesc, tmpValue, descs, values] = [[], [], [], [], []];

  if (inputsArray.length > 0) {
    for (let i = 0; i < inputsArray.length; i++) {
      const RegExp3 = new RegExp(`\\b${inputsArray[i]}\\[.*?]"`, 'igm');
...
function InputModal(props) {
  ...
  useEffect(() => {
    ...
    http
      ...
      .then(res => {
        const definition = JSON.stringify(res.result, null, 2);
        const description = res.result?.description?.split('-')[0] || '';
        const labels = getInputs(definition);
        const {descs, values} = getDetails(definition, labels);
  ...
  const executeWorkflow = () => {
    const {labels, values} = {...workflowForm};
    const input = {};
    const payload = {
      name: name,
      version: version,
