...
const initialState = {
  ...
  functional: [
    ...
    {
      name: 'decision',
      description: 'Conditional branching point in a workflow',
    },
    ...
    {name: 'wait', description: 'Wait for a specific event before continuing'},
    {
      name: 'event',
      description:
        'Publish a specific event (other workflow might be waiting for)',
    },
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case UPDATE_FINAL_WORKFLOW: {
      const {finalWorkflow} = action;
      return {...state, finalWorkflow};
    }
    case SHOW_CUSTOM_ALERT: {
      const {show, variant, msg} = action;
